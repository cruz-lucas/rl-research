...
class DTRMaxNStepAgent(TabularAgent[DTRMaxNStepState, DTRMaxNStepParams]):
    ...
    def select_action(
        ...
        plan_values = self._plan_targets(state.q_values, state.sa_counts, obs_idx, state.timestep, plan_key)
    ...
    def _plan_targets(
        self, q_values: jax.Array, sa_counts: jax.Array, obs_idx: jax.Array, timestep: jax.Array, rng: jax.Array
    ) -> jax.Array:
        ...
        def rollout_target(action_idx: jax.Array, key: jax.Array) -> jax.Array:
            return self._rmax_n_step_return(
                q_values, sa_counts, obs_idx, action_idx, timestep, key
            )
    ...
    def _rmax_n_step_return(
        ...
        def body(step: jax.Array, carry):
            ...
            bonus = self._bonus_value(sa_counts, next_state, timestep)
            action_values = q_values[next_state] + self._beta * bonus
    ...
    def _bonus_value(
        ...
        counts = visit_counts[state_idx]
        safe_counts = jnp.maximum(counts, jnp.ones_like(counts, dtype=jnp.float32))
