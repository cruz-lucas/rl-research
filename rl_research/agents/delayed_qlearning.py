...
class DelayedQLearningAgent:
    ...
    def update(
        ...
        def update_body(i, carry):
            ...
            pending_returns = pending_returns.at[s, a].add(
                jnp.where(can_learn, target, 0.0)
            )
            ...
            def apply_update(_):
                ...
                updated_flags = jax.lax.cond(
                    decrease, lambda f: f, lambda f: f.at[s, a].set(False), learn_flags
                )
            ...
            (
                ...
                change,
                update_flag,
            ) = jax.lax.cond(ready, apply_update, skip_update, operand=None)
        ...
        (
            ...
            total_updates,
            success_any,
        ) = jax.lax.fori_loop(0, batch_size, update_body, init_carry)
    ...
    def bootstrap_value(
        self, state: DelayedQLearningState, next_observation: jnp.ndarray
    ) -> jax.Array:
