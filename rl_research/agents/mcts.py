...
class MCTSAgent:
    ...
    def initial_state(self) -> MCTSState:
        ...
        return MCTSState(
            transition_counts=jnp.zeros(
                (self.num_states, self.num_actions, self.num_states), dtype=jnp.float32
            ),
            reward_sums=jnp.zeros(
    ...
    def _sample_model(
        self, state: MCTSState, s: jnp.ndarray, a: jnp.ndarray, key: jax.Array
    ):
    ...
    def _run_rollout(
        ...
        def body_fn(i, carry):
            ...
            return jax.lax.cond(
                i < depth, do_roll, lambda carry_in: carry_in, (s, k, ret, disc)
            )
    ...
    def select_action(
        ...
        def simulation(carry, _):
            ...
            def selection_step(i, sel_carry):
                ...
                def do_step(carry_in):
                    ...
                    exploration = self.ucb_c * jnp.sqrt(
                        jnp.log(total_visits + 1) / jnp.maximum(sa_visits, 1)
                    )
                ...
                return jax.lax.cond(
                    jnp.logical_and(
                        jnp.logical_not(exp_flag), p_len < self.rollout_depth
                    ),
                    do_step,
            ...
            rollout_return, key_roll = self._run_rollout(
                state, current_state, remaining_depth, key_roll
            )
        ...
        action_values = tree_values[root_state] / jnp.maximum(
            tree_visits[root_state], 1
        )
        action = _select_greedy(action_values, key)
        return action

    def update(
        self, state: MCTSState, batch: Transition
