...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--experiment-name", default="riverswim_rmax_dtp_sweep")
...
def build_environment() -> RiverSwimJaxEnv:
...
def build_sweep_points(args: argparse.Namespace) -> list[SweepPoint]:
    ...
    for idx, (lr, m, horizon) in enumerate(grid, start=1):
        label = _make_label(lr, m, horizon)
        overrides = {
            "learning_rate": lr,
            "m": m,
            "horizon": horizon,
...
def _make_label(lr: float, m: float, horizon: int) -> str:
    def _fmt(value: float | int) -> str:
        text = f"{value}"
        return text.replace(".", "p").replace("-", "m")

    return f"lr{_fmt(lr)}_m_{_fmt(m)}_hz{_fmt(horizon)}"
...
def main() -> None:
    ...
    for point in selected:
        ...
        agent_params = DTRMaxNStepParams(**base_agent_kwargs | point.overrides)
